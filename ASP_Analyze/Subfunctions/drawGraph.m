...
if auto == 0
    prompt = {'Direction:','Nr in Sequence','Blank?','Blip?','# of Block','# of Profile','Blank Good?','Blip Good?'};
    dlg_title = 'Graph-Parameters';
    num_lines = 1;
    defaultans = {'1','[2 3 4 5]','1','0','0','0','2','2'};
...
paraName = {'Dr';'NiS';'blank';'blip';'NoB';'NoP';'gBlank';'gBlip'};
% plot target
...
for s = 1:8
    uicontrol('Style','text','String',permute(plotData.para(numPlot,s,1:lAnsw(s)),[3 2 1])','Visible','on','units','normalized','Position', [0.70+numPlot/20 0.90-0.025*(s-1) 0.05 0.025]);
end

plotData.avData(numPlot,1)= NoGT;
plotData.avData(numPlot,2)= 100*NoGT/NoAT;

uicontrol('Style','text','String',plotData.avData(numPlot,1),'Visible','on','units','normalized','Position', [0.70+numPlot/20 0.625 0.05 0.025]);
uicontrol('Style','text','String',round(plotData.avData(numPlot,2)),'Visible','on','units','normalized','Position', [0.70+numPlot/20 0.600 0.05 0.025]);
...
for s = 1:7
    if s == 4
    uicontrol('Style','text','String',round(plotData.avData(numPlot,6)),'Visible','on','units','normalized','Position', [0.70+numPlot/20 0.55-0.025*(s-1) 0.05 0.025]);    
    else    
    uicontrol('Style','text','String',plotData.avData(numPlot,s+2),'Visible','on','units','normalized','Position', [0.70+numPlot/20 0.55-0.025*(s-1) 0.05 0.025]);
    end
end    
axes(ax(1));
